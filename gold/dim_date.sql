...
SELECT
    ...
    CASE WHEN DAYOFWEEK(d.date) BETWEEN 2 AND 6 THEN TRUE ELSE FALSE END AS is_weekday,
    CASE WHEN DAYOFWEEK(d.date) IN (1, 7) THEN TRUE ELSE FALSE END       AS is_weekend,
    CASE WHEN d.date = LAST_DAY(d.date) THEN TRUE ELSE FALSE END         AS is_last_day_of_month,
    CASE WHEN MONTH(d.date) >= 7 THEN YEAR(d.date) + 1 ELSE YEAR(d.date) END AS fiscal_year,
    CASE 
        WHEN MONTH(d.date) IN (7,8,9)    THEN 1
        WHEN MONTH(d.date) IN (10,11,12) THEN 2
        WHEN MONTH(d.date) IN (1,2,3)    THEN 3
        ELSE 4 
    END AS fiscal_quarter,
    CASE 
        WHEN MONTH(d.date) >= 7 THEN MONTH(d.date) - 6
        ELSE MONTH(d.date) + 6 
    END AS fiscal_month
FROM (
    SELECT EXPLODE(SEQUENCE(TO_DATE('2010-01-01'), TO_DATE('2030-12-31'), INTERVAL 1 DAY)) AS date
) d;